...
**关联**：`server.py` 里 `histories`/`current`/`char_name`/`other` 已经在角色ID改名的同一批里从 `"mzk"` 换成了 `"mizuki"`（保持二元 if/else 结构不变），但 `call_character_async()` 里调用的还是 `get_mzk_system`/`get_mzk_system_compat` 这两个别名——这也是它们暂时还不能删的原因。
