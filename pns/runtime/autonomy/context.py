...
DIALOGUE_OUTPUT_RULES = (
    ...
    "角色平时可能做某件事，不等于此刻正在做；当前上下文没有明确提供活动时，"
    "不要自行声称正在画画、作曲、剪辑、工作或进行其他具体活动。",
    "判分时，角色设定上合理或常见不算上下文证据：台词若把角色视角事实与最近"
    "对话都未提供的具体当前活动、过去事件或完成状态说成事实，"
    "unsupported_invention 必须达到拒绝阈值；明确的疑问或猜测不算事实断言。",
)
