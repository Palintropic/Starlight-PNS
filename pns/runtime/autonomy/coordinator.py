...
class AutonomousRuntime:
    ...
    def lifecycle_boundary(self):
        ...
        持有期间没有任何提交能落地，也没有 start/stop 能翻转运行标志，所以块
        内看到的状态是一份一致快照。代价对称：块内的活会挡住那些操作，因此
        块里只许放确定性的纯内存工作（取快照），绝不放 I/O 或模型调用。

        事务**内部**进来一律拒绝。闸门是可重入锁，同一个线程再进来会被放行，
        而那一刻的世界是半截的 —— 放行等于允许存下一份不存在过的世界。
        """
        with self._gate:
            ...
            if self._state.in_transaction:
                # 闸门到手了，会话上却还有一次提交开着 —— 那次提交不是协调器
                # 发起的（调度器的时间推进、事件提交层，都能直接开事务）。
                # 闸门管不到它们，所以这里必须再问一次会话本人。
                #
                # 拒绝而不是等：等需要在会话上架一套条件变量，而这条路上的
                # 正确做法本来就是"通过 advance() 推时间"——那条路持着闸门，
                # 根本不会走到这里。响亮失败会把绕开闸门的驱动方指出来，
                # 而静默等待只会把它藏起来。
                raise AutonomyError(
                    "会话上有一次提交正在进行（不是由这个协调器发起的），"
                    "此刻取快照会存下一份半截世界。推进时间请走 advance()"
                )
            yield self
