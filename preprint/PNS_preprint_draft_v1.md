...
### 3.5 Character pack architecture (pluggable)

The closed-world container and Router described above are implemented independently of any specific character roster. Character identity, canon facts, and system-prompt material are factored out into a **character pack**: a self-contained, versioned directory (`packs/pjsk/`) of per-character YAML metadata plus unit-level grouping, loaded at runtime by a registry component rather than hard-coded into the framework. The current pack registers 20 characters across five in-fiction units from the PJSK/Project SEKAI franchise; of these, only Ena and mizuki (from the unit 25時、ナイトコードで。) are presently built out with the dialogue samples and canon detail needed to serve as experimental subjects, with the remaining 18 registered as placeholders for future extension. This separation means the closed-world container and two-layer Router are not specific to the Ena/mizuki pair and could in principle be pointed at a different pack — including a non-PJSK one — without changes to the scoring logic itself; we have not yet tested this generality empirically and flag it as future work (Section 7).

---

## 4. Case Study: Three-Round Correction of Language Density Drift

As an illustrative demonstration of correction compliance measurement, we report a three-round correction sequence for the mizuki character, each round re-scored by both the Router and a human annotator:
...
**Intellectual property.** The two characters used as experimental subjects (Ena and mizuki, from the unit 25時、ナイトコードで。within the PJSK/Project SEKAI franchise) are the intellectual property of SEGA and Colorful Palette. The framework's character pack (Section 3.5) additionally registers a broader roster of PJSK characters for future extensibility; these are not currently used as experimental subjects but are drawn from the same franchise and are likewise SEGA/Colorful Palette IP. They are used here strictly as fixed, well-documented fictional personas that provide a convenient closed world with pre-existing canon (schedules, relationships, established voice) for controlled experimentation, not as a claim of ownership or affiliation. No copyrighted game assets, art, or verbatim dialogue are reproduced in this paper or the associated codebase.
...
- **Cross-pack generality.** The character pack architecture (Section 3.5) decouples the framework from the Ena/mizuki pair by design, but we have not yet run the closed-world container and Router against any of the other 18 registered characters, other units within the same pack, or a non-PJSK pack. Testing whether drift-scoring behavior generalizes across packs is planned future work.
